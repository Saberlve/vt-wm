...
default_livestream = 0 if getattr(args_cli, "headless", False) else 2
args_cli.livestream = int(os.environ.get("UNIVTAC_LIVESTREAM", default_livestream))
args_cli.num_envs = 1
...
def main():
    ...
    env_cfg.record_pre_move_video = task_config.get("record_pre_move_video", env_cfg.record_pre_move_video)
    ...
    success_rate = results['succ_num'] / results['test_num'] * 100 if results['test_num'] else 0.0
    log(f"Final Result: {results['succ_num']}/{results['test_num']}({success_rate:.2f}%) success.")
