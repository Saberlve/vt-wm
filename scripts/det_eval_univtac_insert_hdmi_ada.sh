#!/bin/bash
# Determined entrypoint: 2-GPU UniVTAC insert_HDMI closed-loop VT-WM evaluation on ada-24g.
set -euo pipefail
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/ada24g_univtac_insert_hdmi_eval_$(date +%Y%m%d_%H%M%S).log"

{
  ...
  scripts/run_univtac_eval.sh \
    --deploy-config VTWM/deploy_insert_hdmi \
    --task insert_HDMI \
